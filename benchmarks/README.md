...
**Report-only policy:** runners are for local comparison and regression notes. `bench:fps` and `bench:lighthouse` exit with code **0** even when FPS or scores are low; they exit with code **1** only when the harness fails (missing `dist/`, preview server, browser, scroll root, or Lighthouse crash). Vitest bench and bundle recording follow normal npm exit codes for test/harness failures only — low numbers are not failures.
