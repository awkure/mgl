...
- [x] Подтянуть screenshots / videos из профиля как вложения заметки (CLI)

### Статус и playtime

- [x] Эвристики статуса из `playtime_forever` / `playtime_2weeks`
      (0 → wishlist/unplayed, недавняя активность → playing)
- [x] Не перезаписывать уже выставленный tier без подтверждения
- [x] Показать playtime в UI (часы) — отдельное поле или только при импорте
