...
import type { SteamAppDetailsSlice } from "../domain/steamImport";
...
async function fetchSteamStoreDetails(appid: number): Promise<SteamAppDetailsSlice> {
  let response: Response;
  try {
    response = await fetch(steamAppDetailsApiUrl(appid));
  } catch {
    throw new Error(steamPrefillFetchErrorMessage(appid));
  }
  if (!response.ok) {
    throw new Error(steamPrefillFetchErrorMessage(appid, `Storefront HTTP ${response.status}.`));
  }
  const body = await response.json();
  const details = steamAppDetailsFromStoreJson(appid, body);
  if (!details) {
  ...
  useLayoutEffect(() => {
    ...
    const layoutFrame = window.requestAnimationFrame(() => {
      settleFrame = window.requestAnimationFrame(() => {
        ...
        const textarea = editor?.querySelector<HTMLTextAreaElement>('textarea[aria-label="Текст заметки"]');
        if (!editor || !textarea) return;
        textarea.focus({ preventScroll: true });
        editor.scrollIntoView?.({ behavior: "auto", block: "nearest", inline: "nearest" });
      });
    });
    return () => {
...
function NoteGroupAddButton({ groupRank, label, text, disabled, onCreate }: {
  groupRank?: number;
  label: string;
  text: string;
  disabled: boolean;
        ...
        <aside aria-label={game.title} className="game-sidebar">
          ...
          <dl className="game-sidebar__meta">
            ...
            {game.steamAppId != null ? (
              <div><dt>Steam</dt><dd><a className="game-sidebar__import-link" href={steamStoreAppUrl(game.steamAppId)} rel="noreferrer" target="_blank">Steam</a></dd></div>
            ) : null}
            <div><dt>Импорт через</dt><dd>{IMPORTED_VIA_LABELS[game.importedVia]}</dd></div>
...
function NewGamePage({ assets, platformSuggestions = [], tagSuggestions = [], storageLocked = false, canAddBlob, resolveAssetUrl, onCancel, onSave }: GamePageProps) {
  ...
  const [newDraftNoteId, setNewDraftNoteId] = useState<string | null>(null);
  ...
  const addDraftNote = (groupRank: number, files: File[] = []) => {
    ...
    if (!files.length) setNewDraftNoteId(clientId);
