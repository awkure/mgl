...
describe("notes shelf CSS", () => {
  it("uses an unlimited measured grid with explicit shelf tracks", () => {
    const container = declarationsIn(styles, ".notes-list, .note-editors-grid");
    const cards = declarationsIn(styles, ".note-card");
  ...
  it("collapses the shelf to one column on narrow screens", () => {
    expect(styles).toMatch(
      new RegExp(
        `@media \\(max-width: 500px\\)[\\s\\S]*?${cssSelectorPattern(".notes-list, .note-editors-grid")}\\s*\\{\\s*grid-template-columns:\\s*1fr;`,
      ),
    );
  });

  it("keeps labelled add actions in the normal flow below each group", () => {
    const groups = declarationsIn(styles, ".note-groups");
    const group = declarationsIn(styles, ".note-group");
    const groupBoundary = declarationsIn(styles, ".note-group + .note-group");
    const empty = declarationsIn(styles, ".note-empty-group");
    const slot = declarationsIn(styles, ".note-group-add-slot");
    const button = declarationsIn(styles, ".note-group-add-button");
  ...
  it("keeps top-level Markdown lists flush with the note content", () => {
    expect(declarationsIn(styles, ".markdown > ul, .markdown > ol")).toMatch(/padding-inline-start:\s*18px/);
    expect(
      declarationsIn(styles, ".markdown > ul:has(> .markdown-task-item), .markdown > ol:has(> .markdown-task-item)"),
    ).toMatch(/padding-inline-start:\s*0/);
    expect(declarationsIn(styles, ".markdown ul ul, .markdown ul ol, .markdown ol ul, .markdown ol ol")).toMatch(
      /padding-inline-start:\s*18px/,
    );
    expect(declarationsIn(styles, ".markdown-task-item")).not.toMatch(/display:\s*flex/);
    expect(declarationsIn(styles, ".markdown-task-row")).toMatch(/display:\s*flex/);
  });

  it("uses positive green styling for completed checklist rows", () => {
    const completed = declarationsIn(
      styles,
      ".markdown-task-item--checked > .markdown-task-row > .markdown-task-content",
    );

    expect(completed).toMatch(/color:\s*var\(--success\)/);
    expect(completed).not.toMatch(/text-decoration:\s*line-through/);
    expect(declarationsIn(styles, ".markdown-task-checkbox:checked")).toMatch(/accent-color:\s*var\(--success\)/);
  });

  it("keeps checklist progress compact and marks complete headings green", () => {
    const heading = declarationsIn(styles, ".markdown-checklist-heading");
    const title = declarationsIn(styles, ".markdown-checklist-heading__title");
    const progress = declarationsIn(styles, ".markdown-checklist-progress");
    ...
    expect(declarationsIn(styles, ".markdown .markdown-checklist-heading--complete")).toMatch(/color:\s*var\(--success\)/);
  });
});
